...
export function ContentList() {
  ...
  const [option, setOption] = useState('ProyectosWeb')
            ...
            <div key={item.idDoc} className="flex justify-between p-4 mt-5 bg-gray-100 border border-white rounded">
              {
                ...
                <form className="grid w-full gap-10 md:flex" onSubmit={handleGuardarContenido}>
                    <div className='w-full'>
                      ...
                      <div className="pt-1 mb-3 font-medium text-gray-800 font-montserrat">
                        <label htmlFor="yearStart" className="block font-bold text-gray-800 font-montserrat">Fecha Inicio:</label>
                        <select name="yearStart" id="yearStart" className="w-full p-2 border rounded resize-none"  value={contenido.yearStart} 
                      ...
                      <div className="pt-1 mb-3 font-medium text-gray-800 font-montserrat">
                        <label htmlFor="yearEnd" className="block font-bold text-gray-800 font-montserrat">Fecha Fin:</label>
                        <select name="yearEnd" id="yearEnd" className="w-full p-2 border rounded resize-none" value={contenido.yearEnd} 
                                onChange={handleChange}>
                          {years.map((year) => (
                              <option key={year} value={year}>
                                {year}
                ...
                <div className="justify-between w-full gap-10 text-lg font-bold text-gray-800 md:flex font-montserrat">
                  <div className='flex flex-col gap-3 md:w-3/4'>
                    ...
                    <div className="text-gray-600 font-montserrat">{item?.tipoContenido} {'//'} {item.yearStart} {(item.yearEnd && (item.yearEnd !== item.yearStart)) ? `- ${item.yearEnd}` : ''}</div>
                  </div>
